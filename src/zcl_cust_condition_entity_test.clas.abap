CLASS zcl_cust_condition_entity_test DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_cust_condition_entity_test IMPLEMENTATION.
...
*          ls_entity_key-condition_contract  = ls_filter.
        ...
        ls_firstlvl-Condition_contract = es_result-condition_contract.
        ls_firstlvl-Customer = es_result-customer.
        ls_firstlvl-CndnContrType = es_result-cndn_contr_type.
        ls_firstlvl-Supplier = es_result-supplier.
        ls_firstlvl-SalesOrganization = es_result-sales_organization.
        ls_firstlvl-DistributionChannel = es_result-distribution_channel.
        ls_firstlvl-Division = es_result-division.
        ls_firstlvl-AmountFieldGroup = es_result-amount_field_group.
        ls_firstlvl-CndnContrValidFrom = es_result-cndn_contr_valid_from.
        ls_firstlvl-CndnContrValidTo = es_result-cndn_contr_valid_to.
        ls_firstlvl-DocumentReferenceID = es_result-document_reference_id.
        ls_firstlvl-ExternalDocumentReferenceID = es_result-external_document_referenc.
        APPEND ls_firstlvl TO lt_firstlvl.
**********************************************************************
...
*        APPEND ls_final TO lt_final.
        ...
        LOOP AT es_result-cndn_contr_bus_vol_sel_cri INTO DATA(ls_busvol).
          ls_secondlvl-busvolfieldcombntype = ls_busvol-bus_vol_field_combn_type.
          ls_secondlvl-cndncontrbusvolsign = ls_busvol-cndn_contr_bus_vol_sign.
          ls_secondlvl-cndncontrbusvolvalidfrom = ls_busvol-cndn_contr_bus_vol_valid_f.
          ls_secondlvl-cndncontrbusvolvalidto = ls_busvol-cndn_contr_bus_vol_valid_t.
          ls_secondlvl-customer = ls_busvol-customer.
          ls_secondlvl-customerhierarchy = ls_busvol-customer_hierarchy.
          ls_secondlvl-product = ls_busvol-product.
          ls_secondlvl-productgroup = ls_busvol-product_group.
          ls_secondlvl-producthierarchy = ls_busvol-product_hierarchy.
          ls_secondlvl-producttype = ls_busvol-product_type.
          ls_secondlvl-salesorganization = ls_busvol-sales_organization.
          APPEND ls_secondlvl TO lt_secondlvl.
          CLEAR ls_secondlvl.
        ENDLOOP.
        ...
        ls_final-cndn_contr_bus_vol_sel_cri = lt_secondlvl.
        ls_final-cndn_contr_cndn_record = lt_thirdlvl.
        APPEND ls_final TO lt_final.
...
*        IF io_request->get_entity_id(  ) = 'ZI_CONDITION_5'.
*          IF io_request->is_total_numb_of_rec_requested(  ).
*            io_response->set_total_number_of_records( lines( lt_firstlvl ) ).
*          ENDIF.
*          IF io_request->is_data_requested(  ).
*            io_response->set_data( lt_firstlvl ).
*          ENDIF.
*        ELSEIF io_request->get_entity_id(  ) = 'ZI_CNDN_BUSVOL_5'.
*          IF io_request->is_total_numb_of_rec_requested(  ).
*            io_response->set_total_number_of_records( lines( lt_secondlvl ) ).
*          ENDIF.
*          IF io_request->is_data_requested(  ).
*            io_response->set_data( lt_secondlvl ).
*          ENDIF.
*        ELSEIF io_request->get_entity_id(  ) = 'ZI_CNDN_RECORD_5'.
*          IF io_request->is_total_numb_of_rec_requested(  ).
...
*          ENDIF.
*        ELSEIF io_request->get_entity_id(  ) = 'ZI_CNDN_RECORD_5'.
*          DATA(lt_parameters3) = io_request->get_parameters( ).
*          DATA(lt_filter_condition3) = io_request->get_filter( )->get_as_ranges( ).
*          DATA(ls_filter3) = lt_filter_condition3[ 1 ]-range[ 1 ]-low.
*          ls_entity_key-condition_contract  = ls_filter3.
*          DATA(lo_request3) = get_proxy( )->create_resource_for_entity_set( 'CONDITION_CONTRACT' )->navigate_with_key( ls_entity_key )->navigate_to_many( iv_navigation_property_name = 'CNDN_CONTR_CNDN_RECORD' )->create_request_for_read( ).
*
*          DATA(ld_skip_test3) = io_request->get_paging( )->get_offset( ).
*          DATA(ld_top_test3) = io_request->get_paging( )->get_page_size( ).
**          DATA(lr_node) =  lo_request->create_expand_node( ).
**          lr_node->add_expand( iv_expand_property_path =  'CNDN_CONTR_BUS_VOL_SEL_CRI' ).
**          lr_node->add_expand( iv_expand_property_path =  'CNDN_CONTR_CNDN_RECORD' ).
*
*          DATA(lo_response3) = lo_request3->execute( ).
*          lo_response3->get_business_data( IMPORTING et_business_data = lt_thirdlvl ).
*          IF io_request->is_total_numb_of_rec_requested(  ).
*            io_response->set_total_number_of_records( lines( lt_thirdlvl ) ).
*          ENDIF.
*          IF io_request->is_data_requested(  ).
...
*      ENDIF.
        ...
        CLEAR: lo_root_filter_node, lt_secondlvl, lt_thirdlvl.
