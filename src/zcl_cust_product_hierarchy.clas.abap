CLASS zcl_cust_product_hierarchy DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PROTECTED SECTION.
  PRIVATE SECTION.
    TYPES tt_result TYPE TABLE OF ZI_product_hierarchy.
    DATA: et_res TYPE TABLE OF zi_product_hierarchy.
    DATA: es_res TYPE zi_product_hierarchy.
...
CLASS zcl_cust_product_hierarchy IMPLEMENTATION.
    ...
    IF io_request->is_data_requested(  ).
      ...
      LOOP AT lt_filter_condition INTO DATA(ls_filter).
        IF ls_filter-name = 'PRODUCT_SALES_ORG'.
          lv_sales_org = ls_filter-range[ 1 ]-low.
        ENDIF.
        IF ls_filter-name = 'PRODUCT_DISTRIBUTION_CHNL'.
          lv_dist_chnl = ls_filter-range[ 1 ]-low.
        ENDIF.
      ENDLOOP.
      ...
      CALL FUNCTION 'BAPI_MATERIAL_GET_PRODUCTHIER' DESTINATION lo_dest
        EXPORTING
          hierarchy_type       = '1'
          sales_organization   = lv_sales_org
          distribution_channel = lv_dist_chnl
        TABLES
          node_list            = node_list.
*call api to fetch all product
      ...
      LOOP AT node_list INTO DATA(ls_hierarchy).
        ...
        ELSE.
          ...
          APPEND es_res TO et_res.
          CLEAR: es_res.
*Call the API for product to get the description of product.
*          DATA(lo_request) = get_proxy( )->create_resource_for_entity_set( 'A_PRODUCT' )->create_request_for_read( ).
*          DATA(lr_node) =  lo_request->create_expand_node( ).
*          lr_node->add_expand( iv_expand_property_path =  'TO_DESCRIPTION' ).
*          lr_prod = VALUE #( (   sign = 'I' option = 'EQ' low = ls_hierarchy-prod_hier ) ).
*          DATA(lo_filter_factory) = lo_request->create_filter_factory( ).
*          DATA(lo_filter_node) = lo_filter_factory->create_by_range(
*            iv_property_path = 'PRODUCT_HIERARCHY'
*            it_range         = lr_prod
*          ).
